...
sub100 is a component library where perceived speed is the design system. Every interaction acknowledged in under 100ms, every state change legible without waiting. Proving ground: AI interfaces. Reach: any product that refuses to feel slow.
 
This repo is a **pnpm + Turborepo monorepo**:
 
- `apps/www` — Next.js 15 docs site (also serves the component registry at `/r/[namespace]/[name]`)
- `packages/ui` — source-of-truth components (domain-agnostic, ships first)
- `packages/ai` — `@sub100/ai` opt-in AI primitives (placeholder in v0.1, ships in v0.2)
- `scripts/build-registry.ts` — compiles `packages/ui/*` → JSON for the registry
...
Persistent lessons live in [`learnings.md`](./learnings.md) — read it at the start of every task and follow every rule there.
...
`progress.md` is the long-lived map of the project — what's built, in flight, blocked, and planned. The visible task list (§7) is your *short-lived* working memory; `progress.md` is the *durable* one.
...
## 10. Project conventions (the sub100 way)
 
These flow from the thesis. Push back if asked to violate any.
 
- **Press fires on `pointerdown`, not `click`.** Always.
- **CSS transforms only** for press/hover/motion — never layout-triggering properties (width, height, top, left, margin).
- **Optimistic by default.** Mutations show the resolved state immediately, reconcile on resolve. Use `useOptimistic` + `useTransition`. Failures land in a Toast.
- **Motion serves response, not luxury.** Three spring presets in `lib/springs.ts`, all 120–180ms. Don't invent new ones.
- **Tabular numerals + monospace timestamps** in the type scale. Numbers don't shift.
- **Keyboard parity, not keyboard support.** Every component fully operable from the keyboard with visible focus rings.
- **`prefers-reduced-motion` respected automatically** in every motion-using component.
- **No external state libraries in `packages/ui` or `packages/ai`.** React 19 primitives only (`useOptimistic`, `useTransition`, `useState`, `useReducer`, Context sparingly). Zustand/Jotai are consumer concerns.
- **Wrap Radix, ship opinions.** We're not headless. If a primitive exists in Radix, build on it rather than from scratch.
- **RSC-friendly where possible.** Avoid `'use client'` in components that don't need it.

## 11. Installed CLI tools (prefer these)
 
- `pnpm` — the package manager. Never `npm` or `yarn` in this repo.
- `turbo` — task runner. `turbo run build`, `turbo run dev --filter=www`, etc.
- `biome` — lint + format. `biome check`, `biome format --write`.
...
Discover commands instead of memorizing them:
 
```bash
pnpm run                            # list scripts in root package.json
pnpm --filter <name> run            # list scripts for a specific workspace
pnpm -r list --depth -1             # list all workspaces (apps/, packages/)
turbo run --help                    # turbo flags (--filter, --parallel, etc.)
ls scripts/                         # bootstrap / build-registry / etc.
```
 
Standard flow: `pnpm install` → `pnpm dev` (or scope with `--filter=www`). Use `pnpm run` first to see what's actually defined before guessing.